/* GNUI_NSlider for WidgetSet, Copyright (c) 1998 curtis Edwards (curt1@jps.net)
...
void  GNUI_NSlider::setDefaults()
{
  slider_size((float)0.06);
  slider(GNUI_UP_BOX);    
  alignTick(GNUI_ALIGN_TOP);
  majorTickFont(GNUI_HELVETICA);
  majorTickFontSize(9);
  majorTickPrecision(0);
  minorTick(0);
  majorTick(0);
  colorTick( GNUI_BLACK);
  selection_color(9);
  hilight_color(GNUI_RED);
  color(GNUI_GRAY);
...
/////////////////////////////////The New Slider
GNUI_NSlider::GNUI_NSlider(uchar t, int x, int y, int w, int h, const char* l)
: GNUI_Valuator(x, y, w, h, l) {
  type(t);
   switch(type())
   {
        case GNUI_VERT_NICE_SLIDER:
        case GNUI_HOR_NICE_SLIDER	:
             box(GNUI_FLAT_BOX);
             break;
       default: box(GNUI_DOWN_BOX);
             break;
   }
   setDefaults();
}
 //////////////////////////////////The New Slider
GNUI_NSlider::GNUI_NSlider(int x,int y,int w,int h, const char *l)
: GNUI_Valuator(x, y, w, h, l)
{
   box(GNUI_DOWN_BOX);
   setDefaults();
...
void GNUI_NSlider::calcButton(int &knob_x, int &knob_y, int &knob_w , int &knob_h, double val) {
 ...
 switch(type()) {
        case GNUI_VERT_FILL_SLIDER:
             knob_x = (int)px_x;
	     knob_y = y()+box()->dy();
	     knob_w = (int)px_w;
             knob_h = (int)(tk_button_incr-y());
	      break;
        case GNUI_HOR_FILL_SLIDER	:
             knob_x = x()+box()->dx();
	     knob_y = (int)px_y;
	     knob_w = (int)(tk_button_incr-x());
             knob_h = (int)px_h;
	     break;
        case GNUI_VERT_HALF_SLIDER:
             knob_x = (int)px_x;
	     knob_y = (int)(tk_button_incr-(pxSlider/2));
	     knob_w = (int)(px_w/2);
             knob_h = pxSlider;
 	     break;
        case GNUI_HOR_HALF_SLIDER	:
...
void GNUI_NSlider::value_damage(){
  int knob_x, knob_y, knob_w, knob_h;
  if(value() != oldval) 
  {
       calcButton( knob_x, knob_y, knob_w, knob_h,  oldval);
       if(box() == GNUI_NO_BOX)  parent()->damage(~1,  knob_x, knob_y, knob_w+3, knob_h+3);
...
//////////////////////fltk handle stuff (notice GNUI_DRAG has no redraw,handle_drag() does it)
int  GNUI_NSlider::handle(int event)
{
...
void   GNUI_NSlider::drawTicks()
{
 ...
 gnui_get_color( selection_color(), sr1, sg1, sb1);  //get color for ticks

 gnui_font( (GNUI_Font)_tick_font,_tick_fontSize);
 
 gnui_color(_tick_col);
 sprintf(pnt,  _majorTickPrecision, maximum());
 gnui_measure(pnt, fw, fh);                           //find font size
 ...
 { 
       ...
       if( majorTick() ) {   
          ...
          mcount   = 0; 
          fnum     = 0; 
          gnui_line(  px_x+px_w , majorLnStart,  px_x+px_w , majorLnStart+majorLnLen );
          while(mcount < px_w) {
               gnui_line(  px_x+mcount  , majorLnStart, px_x+mcount  , majorLnStart+majorLnLen );
               sprintf(pnt, _majorTickPrecision, fnum+minimum() );
               gnui_measure(pnt, fw, fh); 
               if(_tick_fontSize) gnui_draw(pnt, px_x+mcount-(fw/3), majorFntStart); 
               fnum += fnumIncr;
               mcount += mdev;
         }
     } //(if major)
     if( minorTick()  && _minorTick <= maximum()) {  //do minor ticks
         ...
         mcount = mdev;
         while(mcount < px_w)
         {   gnui_line(  px_x+mcount , minorLnStart,  px_x+mcount , minorLnStart+minorLnLen );  //create main line 
	     mcount += mdev;
         }
     } //(if minor)
 } //(isHoz)
 else  //insure the slider is Vert
 {
       ...
       if( majorTick() ) {   
          ...
          mcount   = 0; 
          fnum     = 0; 
          gnui_line(  majorLnStart, px_y+px_h , majorLnStart+majorLnLen,  px_y+px_h );
          while(mcount < px_h) {
               gnui_line( majorLnStart, px_y+mcount  ,majorLnStart+majorLnLen, px_y+mcount );
               sprintf(pnt, _majorTickPrecision, fnum+minimum() );
               gnui_measure(pnt, fw, fh); 
               if(_tick_fontSize) gnui_draw(pnt, majorFntStart,px_y+mcount+(fh/3)); 
               fnum += fnumIncr;
               mcount += mdev;
         }
     } //(if major)
     if( minorTick()  && _minorTick <= maximum()) {  //do minor ticks
         ...
         mcount = mdev;
         while(mcount < px_h)
         {   gnui_line(  minorLnStart, px_y+mcount,  minorLnStart+minorLnLen, px_y+mcount );  //create main line 
	     mcount += mdev;
         }
     } //(if minor) 
 } //else vert
...
{
 GNUI_NSlider* v = (GNUI_NSlider*)vv;
...
void   GNUI_NSlider::drawRamp()
{
  ...
  gnui_draw_image(generate_image, this, x1, y1, w1, h1);
}
//////////////////set the slider size
void GNUI_NSlider::slider_size(float v) {
  if (v <  0) v = 0;
  if (v > 1) v = 1;
  if (slider_size_ != float(v)) {
    slider_size_ = float(v); 
    damage(GNUI_DAMAGE_EXPOSE);
  }
}
////////////////set the bounds (from old slider)
void GNUI_NSlider::bounds(float a, float b) {
  if (minimum() != a || maximum() != b) {
    GNUI_Valuator::range(a, b); 
    damage(GNUI_DAMAGE_EXPOSE);
  }
}
