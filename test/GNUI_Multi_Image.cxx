// Test of the new GNUI_MultiImage class for labelling a widget

#include <fltk/Fl.h>
#include <fltk/GNUI_Window.h>
#include <fltk/GNUI_Button.h>
#include <fltk/GNUI_Check_Button.h>
#include <fltk/GNUI_Multi_Image.h>

extern GNUI_Multi_Image multiimage; // defined below

void inactive_cb(GNUI_Widget* w, void*)
{
  if (w->value()) {
...
{
  GNUI_Window window(200,100);
  for (int i = 0; i < 4; i++) {
    GNUI_Button* b = new GNUI_Button(20+i*40, 20, 32, 32);
    b->image(multiimage);
    // must remove box so it does not blink:
    b->box(GNUI_NO_BOX);
    // remove focus box because we have an image to show focus:
    b->focus_box(GNUI_NO_BOX);
    // you have to set highlight color to get it to redraw when the
    // highlighting is changed:
    b->highlight_color(GNUI_GRAY);
  }
  GNUI_Check_Button check(20,60,180,20,"Make two of them inactive");
...
GNUI_Pixmap up_focus(clear_tick_up_focus);
...
GNUI_Pixmap dn_focus(clear_tick_dn_focus);
...
GNUI_Bitmap up_inactive(clear_tick_mask_bits, clear_tick_mask_width, clear_tick_mask_height);
#define dn_inactive up_inactive

GNUI_Multi_Image multiimage(up,
			 GNUI_HIGHLIGHT, hi,
  			 GNUI_SELECTED, up_focus,
  			 GNUI_SELECTED|GNUI_HIGHLIGHT, dn_focus,
			 GNUI_VALUE, dn,
			 GNUI_INACTIVE, up_inactive,
			 GNUI_INACTIVE|GNUI_VALUE, dn_inactive);

// end of GNUI_MultiImage test
