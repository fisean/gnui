...
GLContext gnui_current_glcontext;
static const Window* cached_window;

void gnui::set_gl_context(const Window* window, GLContext context) {
  if (context != gnui_current_glcontext || window != cached_window) {
    gnui_current_glcontext = context;
...
#elif defined(__APPLE__)
  // warning: the Quartz version should probably use Core GL (CGL) instead of AGL
  if (gnui_current_glcontext) aglSetCurrentContext(0);
#endif
  gnui_current_glcontext = 0;
  cached_window = 0;
}

void gnui::delete_gl_context(GLContext context) {
  if (gnui_current_glcontext == context) no_gl_context();
