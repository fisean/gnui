...
void GlOverlay::draw() {
  if (!valid_) glClearIndex((GLfloat)gnui_transparent_pixel);
...
void GlOverlay::create() {
  ...
  context(create_gl_context(gnui_overlay_visual), 1);
  valid(0);
}

bool GlWindow::can_do_overlay() {
  if (gnui_find_overlay_visual()) return true;
#if 0
// This may work if SERVER_OVERLAY_VISUALS is not set in X server, but glX
// is still able to locate an overlay. This should be in gnui_overlay_visual.cxx
  static int list[] = {GLX_LEVEL, 1, 0};
  gnui_overlay_visual = glXChooseVisual(xdisplay, xscreen, list);
  if (gnui_overlay_visual) return true;
#endif
  return false;
}
...
void GlWindow::make_overlay() {
  ...
  if (!pfd.iPixelType) {
    ...
    gnui_overlay_depth = pfd.cColorBits; // used by gl_color()
    if (gnui_overlay_depth > 8) gnui_overlay_depth = 8;
    COLORREF palette[256];
    int n = (1<<gnui_overlay_depth)-1;
