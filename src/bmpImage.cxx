...
bool bmpImage::fetch()
{
  ...
  if (info_size < 40) {
    ...
    unsigned w = read_dword();
    if (w&0x80000000) w = (w^0xffffffff)+1;
    unsigned h = read_dword();
    if (h&0x80000000) {h = (h^0xffffffff)+1; flip=true; }
    read_word();
    bitsperpixel = read_word();
  ...
  setpixels(array, Rectangle(w(), h()), depth*w());
