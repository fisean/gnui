...
/*! \fn bool gnui_theme();
  ...
  This is the default value of gnui::theme().

  This is declared as a "C" function to allow it to be replaced with
  a function from a loaded plugin. If you are statically linking gnui
  you can also override this by declaring a gnui_theme() function in
  your own code (some Unix systems, but not Linux, will do this even
  if gnui is a shared library).
  ...
  SystemParametersInfo(). FLTK will call gnui::reload_theme() when a
  WM_SYSCOLORCHANGE, WM_DISPLAYCHANGE, or WM_SETTINGCHANGE event
  happens, even if you replace gnui_theme(), so your replacement
...
#define BUTTONCOLOUR gnui::GRAY75
#define BARCOLOUR    gnui::lerp(gnui::GRAY75, 0x8EA2E200, .1)
...
static unsigned char const * const frames[26] = {
	...
	frame26};
static gnui::Image* img_frame, *images[26];
static gnui::Image* pixie = 0, *pixie_hilight, *pixie_return_butt,
...
static void pixie_box_init_check() {
  if (!xp_glyph_initialised) {
    ...
    pixie_hilight = gnui::bmpImage::get("xp_button_hi", xp_button_hi);
    pixie_return_butt = gnui::bmpImage::get("xp_button_return", xp_button_return);
    pixie_down = gnui::bmpImage::get("xp_button_dn", xp_button_dn);
    pixie = pixie_norm = gnui::bmpImage::get("xp_button_up", xp_button_up);
  }
}
...
static void scroll_glyph_init_check() {
  if (!scroll_glyph_initialised) {
    ...
    img_frame = new gnui::Image(FRAME_SIZE, FRAME_SIZE, "current_image");
    char buffer[10];
    for (int i = 0; i < 26; i++){
      if (i < 10)
      ...
      images[i] = gnui::gifImage::get(buffer, frames[i]);
    }
  }
}
...
static void draw_arrows(const gnui::Rectangle& R, int drawflags) {
  if (drawflags & gnui::ALIGN_TOP) {
    if (drawflags & gnui::PUSHED)
      set_frame(V_UP_ARROW_PUSHED);
    else if (drawflags & gnui::HIGHLIGHT)
      set_frame(V_UP_ARROW_HI);
    else
      set_frame(V_UP_ARROW);
    img_frame->draw(R);
  } else if (drawflags & gnui::ALIGN_BOTTOM) {
    if (drawflags & gnui::PUSHED)
  ...
  } else if (drawflags & gnui::ALIGN_LEFT) {
    if (drawflags & gnui::PUSHED)
    ...
    img_frame->draw(R);
  } else if (drawflags & gnui::ALIGN_RIGHT) {
    if (drawflags & gnui::PUSHED)
...
static class XPScrollBarGlyph : public gnui::Symbol {
  public:
    void _draw(const gnui::Rectangle& R) const {
      scroll_glyph_init_check();
      if (gnui::drawflags() & gnui::ALIGN_MASK) {
	draw_arrows(R, gnui::drawflags());
	if (gnui::drawflags() & gnui::ALIGN_BOTTOM) {
	  if (R.h() > R.w()) {
            set_frame(V_TRACK);
	...
	if (R.x() == 0) { // Vertical
	  if (gnui::drawflags() & gnui::PUSHED)
	    set_frame(V_SLIDER_PUSHED);
	  else if (gnui::drawflags() & gnui::HIGHLIGHT)
    ...
    XPScrollBarGlyph() : gnui::Symbol("XPScrollBarGlyph") {}
    ~XPScrollBarGlyph() {
      delete img_frame;
    }
} xp_glyph;

static class ArrowGlyph : public gnui::Symbol {
  public:
    void _draw(const gnui::Rectangle& R) const {
      draw_arrows(R, gnui::drawflags());
    }
    ArrowGlyph() : gnui::Symbol("arrow glyph") {}
} arrow_glyph;

static class NoGlyph : public gnui::Symbol {
  public:
    void _draw(const gnui::Rectangle& r) const{}
    NoGlyph() : gnui::Symbol("no_glyph") {}
} no_glyph;

class NewFocusBox : public gnui::Symbol {
  public:
    NewFocusBox() : gnui::Symbol("NewFocusBox") {}
    void _draw(const gnui::Rectangle& R) const {
      gnui::Rectangle r(R);
      r.inset(3);
      gnui::DEFAULT_FOCUS_BOX->_draw(r);
    }
} new_focusbox;

static class XPBox : public gnui::Box {
  protected:
    const char* data_;
    const gnui::Box* down_;
    bool type_return_;
  public:
    ...
    void _draw(const gnui::Rectangle& R) const {
      ...
      if (gnui::drawflags() & gnui::PUSHED || (gnui::drawflags() & gnui::STATE))
        pixie = pixie_down;
      else if (gnui::drawflags() & gnui::HIGHLIGHT)
        pixie = pixie_hilight;
    ...
    XPBox(const char* name, int dx, int dy, int dw, int dh,
          const char* pattern, const gnui::Box* down = 0) : gnui::Box(name),
...
extern "C" bool xp_theme() {
  ...
  if ((s = gnui::Style::find("PopupMenu"))) {
    s->glyph(&arrow_glyph);
    s->buttonbox(&xp_box);
    s->highlight_color(s->color());
  }
  ...
  if ((s = gnui::Style::find("Button"))) {
    s->buttonbox(&xp_box);
    s->highlight_color(gnui::WHITE);
    s->focusbox(&new_focusbox);
    s->glyph(&no_glyph);
  }
  if ((s = gnui::Style::find("ReturnButton"))) {
    s->buttonbox(&xp_return_box);
    s->highlight_color(gnui::WHITE);
    s->focusbox(&new_focusbox);
    s->glyph(&no_glyph);
  }
  if ((s = gnui::Style::find("Adjuster"))) {
    s->buttonbox(&xp_box);
    s->color(BGCOLOUR);
  ...
  if ((s = gnui::Style::find("InputBrowser"))) {
    s->glyph(&arrow_glyph);
    s->highlight_color(s->color());
  }
  if ((s = gnui::Style::find("ValueInput"))) {
    s->glyph(&arrow_glyph);
    s->color(PAPERCOLOUR);
    s->buttoncolor(SLIDERCOLOUR);
    s->highlight_color(s->color());
  }
  if ((s = gnui::Style::find("CycleButton"))) {
    s->box(&xp_box);
    s->highlight_color(s->color());
  }
  if ((s = gnui::Style::find("Menu"))) {
    s->color(BGCOLOUR);
    s->highlight_color(gnui::DARK_BLUE);
  }
  if ((s = gnui::Style::find("BarGroup"))) {
    s->glyph(&arrow_glyph);
    s->highlight_color(s->color());
  }
