...
int GNUI_Menu_Item::size() const {
  const GNUI_Menu_Item* m = this;
  int nest = 0;
  for (;;) {
    if (!m->text) {
      if (!nest) return (m-this+1);
      nest--;
    } else if (m->flags & GNUI_SUBMENU) {
      nest++;
    }
    m++;
  }
...
const GNUI_Menu_Item* GNUI_Menu_Item::next(int n) const {
  ...
  while (n>0) {
    ...
    m++;
    if (!nest && !(m->flags & GNUI_MENU_INVISIBLE)) n--;
  }
  return m;
}
...
static const GNUI_Menu_Item* add(Group* g, const GNUI_Menu_Item* m,void* data) {
  Group* saved = Group::current();
  g->begin();
  while (m && m->text) {
    Widget* o;
    const GNUI_Menu_Item* next = m+1;
    if (m->flags & (GNUI_SUBMENU | GNUI_SUBMENU_POINTER)) {
      ItemGroup* g = new ItemGroup(m->text); o = g;
      if (m->flags & GNUI_SUBMENU_POINTER) {
	add(g, (GNUI_Menu_Item*)(m->user_data_), data);
      } else {
	next = add(g, next, data) + 1;
      ...
      if (m->flags & GNUI_MENU_RADIO) o->type(Item::RADIO);
      else if (m->flags & GNUI_MENU_TOGGLE) o->type(Item::TOGGLE);
    }
    ...
    if (m->flags & GNUI_MENU_DIVIDER) new Divider();
    m = next;
  }
  Group::current(saved);
  return m;
...
void GNUI_Menu_Item::add_to(Menu* menu, void* data) const {
  ::add(menu, this, data);
}

/* Emulate old popup and test-shortcut methods on GNUI_Menu_Item arrays: */
#include <gnui/PopupMenu.h>
#include <gnui/events.h>

const GNUI_Menu_Item*
GNUI_Menu_Item::pulldown(int X, int Y, int W, int H,
		       const GNUI_Menu_Item* picked,
...
// and returns it:
const GNUI_Menu_Item*
GNUI_Menu_Item::test_shortcut() const
{
  Group::current(0);
  Menu menu(0,0,0,0);
  add_to(&menu);
