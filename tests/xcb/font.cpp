...
)
{
  ...
  if (error)
  {
    std::cerr << "ERROR: " << errMessage << ' ' << error->error_code << '\n';
...
)
{
  ...
  testCookie(textCookie, connection, (char *)"can't paste text");

  /* free the gc */
  xcb_void_cookie_t gcCookie = xcb_free_gc(connection, gc);
  testCookie(gcCookie, connection, (char *)"can't free gc");
}
...
)
{
    ...
    testCookie(gcCookie, connection, (char *)"can't create gc");

    /* close font */
    fontCookie = xcb_close_font_checked(connection, font);
    testCookie(fontCookie, connection, (char *)"can't close font");
    return gc;
}
...
main()
{
  ...
  testCookie(windowCookie, connection, (char *)"can't create window");
  xcb_void_cookie_t mapCookie = xcb_map_window_checked(connection, window);
  testCookie(mapCookie, connection, (char *)"can't map window");
