...
// make the widget browser in the main gnuid window, items use the list method defined by widgetlist
...
gnui::Widget* Widget_List::child(const gnui::Menu*, const int* indexes, int level) {
  ...
  if (item->pixmapID()>0) 
      widget->image(gnuid_pixmap[item->pixmapID()]);
