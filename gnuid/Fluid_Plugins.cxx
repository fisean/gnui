...
// "$Id$"
//
// Plugins code for the Fast Light Tool Kit (GNUI).
//
...
static void ReadPlugin(char* s, const char* location)
{
  ...
  if(!strcmp(s+strlen(s)-sizeof(PLUGINS_EXTENSION)+1, PLUGINS_EXTENSION)) {
    ...
    Fluid_Plugin* d = (Fluid_Plugin*)load_plugin(s2, "gnuid_plugin");
...
#ifndef _WIN32
  char s[256];
  sprintf(s, "%s/.gnuid/plugins/", getenv("HOME"));
  ReadPlugins(s);
  ReadPlugins("/usr/local/lib/gnui/gnuid/");
#endif
}
