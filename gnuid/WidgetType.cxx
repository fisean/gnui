...
// "$Id$"
//
// Widget type code for the Fast Light Tool Kit (GNUI).
//
...
extern gnui::StyleSet* gnuid_style_set;
...
// initialized parts of the widget that are nyi by gnuid.
...
void WidgetType::write_static() {
  ...
  if (!user_class() || !strncmp(subclass, "gnui::", 6)) 
    ::write_declare("#include <gnui/%s.h>", subclass+6);
...
void WidgetType::read_property(const char *c) {
  ...
  if (!strcmp(c,"private")) {
    ...
    // Strip off leading "VERTICAL|gnui::Type::" written by older gnui gnuid:
  ...
  //} else if (!strcmp(c, "divider")) { // do something here...
  ...
  } else if (!strcmp(c, "down_box")) { // ignore this gnui 1.0 item
    read_word();
    ...
    // back compatability with very old gnuid versions that wrote 2 numbers:
...
void live_mode_cb(LightButton*o,void *v) {
  ...
  if (o->value()) {
	...
	live_window->clear_changed();
      delete live_window; // was delete_widget(live_window); but deferred call obsolete in gnui
    }
    live_type = 0L;
    live_widget = 0L;
    live_window = 0L;
