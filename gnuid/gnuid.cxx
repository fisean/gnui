...
// "$Id$"
//
// FLUID main entry for the Fast Light Tool Kit (GNUI).
//
...
"The GNUI user interface designer version 2.0\n"
"Copyright 1998-2006 by Bill Spitzak and others.\n"
"\n"
"This library is free software; you can redistribute it and/or "
"modify it under the terms of the GNU Library General Public "
...
# include <gnui/win32.h>	// for MAX_PATH definition
# include <direct.h>
#else
# include <unistd.h>
# include <sys/stat.h>
...
PrefsData prefs(Preferences::USER, "gnui.org", "gnuid");
int modflag=0;
...
//! gnuid splashscreen implementation:
...
void show_help(const char *name) {
  ...
  if ((docdir = getenv("GNUI_DOCDIR")) == NULL) {
#ifdef __EMX__
    // Doesn't make sense to have a hardcoded fallback
    static char gnui_docdir[1024];

    strlcpy(gnui_docdir, __XOS2RedirRoot("/XFree86/lib/X11/gnui/doc"),
            sizeof(gnui_docdir));
...
# ifdef GNUI_DOCDIR
    docdir = GNUI_DOCDIR;
# else
    docdir = "/usr/local/share/doc/gnui";
# endif
#endif // __EMX__
  }
...
void make_main_window() {
    if (!main_window) {
	Widget *o;
	main_window = new Window(WINWIDTH,WINHEIGHT,"gnuid");
...
/* FIXME : write strings : should it be implemented in gnuid 2 as in gnui1 ?
  if (shell_writemsgs_button->value()) {
    compile_only = 1;
    write_strings_cb(0, 0);
    compile_only = 0;
