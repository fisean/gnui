...
// "$Id"
//
// Fluid Menus implementation for the Fast Light Tool Kit (GNUI).
//
...
MenuBar* build_hierarchy(MenuBar* menubar) {
    ...
    {MenuSection g("&Help");
	new Item("&About gnuid",0,about_cb);
	new Divider();
	new Item("&On gnuid...",0,help_cb);
	new Item("&GNUI Manual...",0,manual_cb);
	new Divider();
...
void fill_in_New_Menu(ItemGroup* menu) {
  newMenu= menu;
  xpmImage** i = gnuid_pixmap;
    newMenu->begin();
	{MenuSection g("code",i[12]);
	  gnuidMenuItem(Functiontype,7);
	  gnuidMenuItem(Codetype, 8);
	  gnuidMenuItem(CodeBlocktype, 9);
	  gnuidMenuItem(Decltype, 10);
	  gnuidMenuItem(DeclBlocktype, 11);
	  gnuidMenuItem(Classtype, 12);
	  gnuidMenuItem(Namespacetype, 49);
	  gnuidMenuItem(Commenttype, 46);
	}
	{MenuSection g("group",i[1]);
	    gnuidMenuItem(Windowtype  , 1);
	    gnuidMenuItem(Grouptype  , 6);
	    gnuidMenuItem(Packtype  , 22);
	    gnuidMenuItem(Tabstype  , 13);
	    gnuidMenuItem(Scrolltype  , 19);
	    gnuidMenuItem(Tiletype  , 20);
	    gnuidMenuItem(WizardGrouptype  , 21);
	    gnuidMenuItem(BarGrouptype  , 17);
	    gnuidMenuItem(StatusBarGrouptype  , 5);
	}
	{MenuSection g("buttons",i[24]);
	    gnuidMenuItem(Buttontype  , 2);
	    gnuidMenuItem(ReturnButtontype  , 23);
	    gnuidMenuItem(LightButtontype  , 24);
	    gnuidMenuItem(CheckButtontype  , 3);
	    gnuidMenuItem(RadioButtontype  , 4);
	    gnuidMenuItem(RepeatButtontype  , 25);
	}
	{MenuSection g("valuators",i[37]);
	    gnuidMenuItem(Slidertype  , 37);
	    gnuidMenuItem(ValueSlidertype  , 39);
	    gnuidMenuItem(ValueInputtype  , 44);
	    gnuidMenuItem(ValueOutputtype  , 45);
	    gnuidMenuItem(Scrollbartype  ,38);
	    gnuidMenuItem(Adjustertype  , 40);
	    gnuidMenuItem(Dialtype  , 42);
	    gnuidMenuItem(ThumbWheeltype  , 43);
	    gnuidMenuItem(ProgressBartype  , 36);
	}
	{MenuSection g("text",i[27]);
	    gnuidMenuItem(Inputtype  , 14);
	    gnuidMenuItem(Outputtype  ,  27);
	    gnuidMenuItem(TextDisplaytype  , 28);
	    gnuidMenuItem(TextEditortype  , 29);
	    gnuidMenuItem(FileInputtype  , 30);
	}
	{MenuSection g("menus",i[17]);
	    gnuidMenuItem(MenuBartype  , 17);
	    gnuidMenuItem(PopupMenutype  , 26);
	    gnuidMenuItem(Choicetype  , 15);
	    gnuidMenuItem(Browsertype  , 31);
	    gnuidMenuItem(InputBrowsertype  , 15);
	    gnuidMenuItem(FileBrowsertype  , 33);
	    gnuidMenuItem(Submenutype  ,18);
	    gnuidMenuItem(Itemtype  , 16);
	    gnuidMenuItem(Dividertype  , 16);
	}
	{MenuSection g("other",i[5]);
	    gnuidMenuItem(Widgettype  , 5);
	    gnuidMenuItem(InvisibleBoxtype  , 5);
	    gnuidMenuItem(HelpViewtype  , 35);
	    gnuidMenuItem(Clocktype  , 34);
	    //helpview 35
	}
      newMenu->end();
}
...
Item * gnuidMenuItem(FluidType& wt,int n) {
  Item* i = new Item(wt.type_name(),0,cb,(void*)&wt);
  if (n >= 0) i->image(gnuid_pixmap[n]);
  return i;
}
